...
const OrderHistory: React.FC<Props> = ({ dates, estDelivery }) => {
  return (
...
const Status: React.FC<{ activity: TOrderActivty, isLast: boolean, estDelivery: string }> = ({ activity, isLast, estDelivery }) => {
  return (
    <div className={`pl-6 ml-2 ${!isLast ? "border-l border-light-border dark:border-main-gray-border border-dashed" : ""} h-[55px] relative`}>
      <div className="absolute top-[-16px]">
        ...
        <p className="text-[15px] text-side-text-light dark:text-side-text-gray">
          ...
          : activity.label === "Processing" ? "Awaiting order confirmation" : `Est. ${estDelivery}`}
        </p>
      </div>
...
const Check: React.FC<{ date: string }> = ({ date }) => {
  return (
    <div className={`outline outline-1 p-[2px] outline-offset-[-1px] outline-bg-primary-btn-hover 
    ${date === "None" ? "bg-nav-light dark:bg-main-gray" : "bg-bg-primary-btn-hover outline"}
