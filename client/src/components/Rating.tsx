...
const Rating: React.FC<Props> = ({ rating, styles }) => {
  return (
    <div className={`text-main-text-white ${getRatingColour(rating)} w-fit rounded-[5px] 
    h-[22px] px-3 flex items-center pb-[1px] gap-[7px] ${styles}`}>
      <p className="text-sm font-semibold">{rating === 0 ? "No reviews" : Math.round((rating + Number.EPSILON) * 10) / 10}</p>
