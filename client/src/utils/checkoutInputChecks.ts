...
export const checkoutInputChecks: {
  ...
  "Address line 1": (address: string): TCheckoutInputReturn => {
    if (address.length === 0) return { valid: false, "message": "Address must not be empty" }
    return { valid: true };
  },
  ...
  "Town/City": (city: string): TCheckoutInputReturn => {
    if (city.length === 0) return { valid: false, "message": "Town/city must not be empty" }
    return { valid: true };
  },
  ...
  "First name": (firstName: string): TCheckoutInputReturn => {
    if (firstName.length === 0) return { valid: false, "message": "First name must not be empty" }
    return { valid: true };
  },
  "Last name": (lastName: string): TCheckoutInputReturn => {
    if (lastName.length === 0) return { valid: false, "message": "Last name must not be empty" }
    return { valid: true };
  },
